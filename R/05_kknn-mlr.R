...
# maximum elevation within a 10-km-diameter circle for each grid cell
grid_h %>% 
  st_as_sf() %>% 
  st_buffer(10000) -> grid_buff

sy_h %>% 
  st_buffer(10000) -> sy_buff

grid_h$mlr <- (over(as_Spatial(grid_buff), grid_h, fn = max) - over(as_Spatial(grid_buff), grid_h, fn = min))$elevation
sy_h$mlr <- (over(as_Spatial(sy_buff), grid_h, fn = max) - over(as_Spatial(sy_buff), grid_h, fn = min))$elevation
