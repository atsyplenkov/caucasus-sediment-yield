...
hc_train <- data.frame(sy = sy$sy[-sample_row],
                       lon = st_coordinates(sy)[-sample_row, 1], 
                       lat = st_coordinates(sy)[-sample_row, 2])

hc_test <- data.frame(sy = sy$sy[sample_row],
                      lon = st_coordinates(sy)[sample_row, 1], 
                      lat = st_coordinates(sy)[sample_row, 2])
...
raster::rasterFromXYZ(dplyr::select(hc_result,
                                    ...
                                    sy),
                      crs = projection(sy)) %>% 
  raster::extract(., as(sy[-sample_row,], "Spatial")) -> hc_train$sy_pred
...
raster::rasterFromXYZ(dplyr::select(hc_result,
                                    ...
                                    sy),
                      crs = projection(sy)) %>% 
  raster::extract(., as(sy[sample_row,], "Spatial")) -> hc_test$sy_pred
...
bind_rows(hc_train %>% mutate(type = "train"),
  ...
  mutate_if(is.numeric, list(~signif(.,3))) -> caucasus_kknn_validate
...
ggplot() +
  ...
  ggsci::scale_color_d3(name = "Посты",
                        guide = guide_legend(title.position = 'top',
                                             ...
                                             label.hjust = 1)) +
  theme_map() +
  theme(legend.position = "bottom") -> ssy_caucasus_kknn
  
# SAVE -----------------------------------------------------------------------
ggsave("figures/3-10_ssy_caucasus_kknn.png",
         plot = ssy_caucasus_kknn,
         dpi = 500, w = 8, h = 6)

ggsave("figures/3-09_caucasus_kknn_validate_graph.png",
         plot = caucasus_kknn_validate_graph,
         dpi = 500, w = 6, h = 4)

save(caucasus_kknn_validate,
